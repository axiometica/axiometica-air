...
RUNBOOK_TOOLS = [
    ...
    # Previously hard-coded to port 443 with `netstat -tuln` (which only reports
    # LISTENING sockets — always zero unless the container itself listened on
    # 443). Rewritten to take {port} as a required parameter and count actual
    # ESTABLISHED connections via `netstat -an`. The old tool row still exists
    # in prod DBs under tool_name 'list_tcp_connections_on_port_443' and is
    # orphaned by this rename — delete it via the tool-catalog UI.
    {   'tool_name': 'count_established_connections',
        'name': 'Count Established TCP Connections on Port',
        'description': 'Count ESTABLISHED TCP connections on a given port (both inbound to a local listener and outbound to a remote endpoint). Uses netstat -an and greps for ESTABLISHED lines on :{port}.',
        'command_variants': {   'docker':     "docker exec {target} sh -c 'netstat -an | awk \"/ESTABLISHED/ && /:{port} /\" | wc -l'",
                                'ssh':        "sh -c 'netstat -an | awk \"/ESTABLISHED/ && /:{port} /\" | wc -l'",
                                'kubernetes': "kubectl exec -n {{namespace}} {{pod_name}} -- sh -c 'netstat -an | awk \"/ESTABLISHED/ && /:{port} /\" | wc -l'",
                                'vcenter':    None,
                                'aws_ssm':    "sh -c 'netstat -an | awk \"/ESTABLISHED/ && /:{port} /\" | wc -l'",
                                'azure':      "sh -c 'netstat -an | awk \"/ESTABLISHED/ && /:{port} /\" | wc -l'",
                                'any':        "sh -c 'netstat -an | awk \"/ESTABLISHED/ && /:{port} /\" | wc -l'"},
        'category': 'diagnostic',
        'blast_radius': 1,
        'requires_approval': False,
        'enabled': True,
        'parameters': [   {   'name': 'port',
                              'type': 'integer',
                              'required': True,
                              'description': 'TCP port number to count established connections on (e.g. 5432 for postgres, 443 for HTTPS, 80 for HTTP).',
                              'default': None}],
        # Single-value output: the count itself. The tool prints just an integer,
        # so kind=regex on `^(\d+)$` reliably captures it regardless of adapter.
        'output_fields': [   {   'field': 'established_count',
                                 'description': 'Number of ESTABLISHED TCP connections on the requested port',
                                 'type': 'integer',
                                 'kind': 'regex',
                                 'pattern': '^(\\d+)$'}]},
...
# ─────────────────────────────────────────────────────────────
# Tools that once shipped in this seed but have been retired.
# Named here so seed_defaults() can delete their rows from existing
# databases on startup — the loader is otherwise upsert-only and would
# leave retired tools as immortal orphans (blocked from UI delete by the
# is_builtin guard, but no longer regenerated by anything).
#
# Only remove tools that are truly gone (replaced, deleted, renamed).
# Never list a tool here to "hide" it — disable it instead so operators
# still see it in the catalog with its history.
# ─────────────────────────────────────────────────────────────
REMOVED_TOOLS: list[str] = [
    # Replaced by 'count_established_connections' — same intent, but
    # port is now a required parameter and the command counts ESTABLISHED
    # connections instead of LISTENING sockets. The old name was misleading
    # ("List TCP Connections on Port 443") — hard-coded to 443 and to
    # listeners, so it silently returned zero for every non-443 service.
    "list_tcp_connections_on_port_443",
]
