...
APPROVED_ACTIONS = [
    ...
    },
    {
        ...
        "command_variants": {
            "docker":     "docker restart {target}",
            "ssh":        "ssh {host} docker compose up -d --scale {target}={replicas}",
            "kubernetes": "kubectl scale deployment {target} --replicas={replicas} -n {namespace}",
            "vcenter":    "docker compose up -d --scale {target}={replicas}",
            "aws_ssm":    "aws ecs update-service --service {target} --desired-count {replicas}",
            "azure":      "az container restart --name {target} --resource-group {resource_group}",
        },
        "category": "remediation_safe",
        "blast_radius": 2,
        "requires_approval": False,
    ...
    },
    {
        ...
        "command": "docker exec {target} psql -U {db_user} -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state='idle' AND state_change < NOW() - INTERVAL '{max_idle_sec} seconds'\"",
        "command_variants": {
            "docker":     "docker exec {target} psql -U {db_user} -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state='idle' AND state_change < NOW() - INTERVAL '{max_idle_sec} seconds'\"",
            "ssh":        "ssh {target} psql -U {db_user} -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state='idle' AND state_change < NOW() - INTERVAL '{max_idle_sec} seconds'\"",
            "kubernetes": "kubectl exec {target} -n {namespace} -- psql -U {db_user} -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state='idle' AND state_change < NOW() - INTERVAL '{max_idle_sec} seconds'\"",
            "vcenter":    "psql -U {db_user} -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state='idle' AND state_change < NOW() - INTERVAL '{max_idle_sec} seconds'\"",
            "aws_ssm":    "psql -U {db_user} -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state='idle' AND state_change < NOW() - INTERVAL '{max_idle_sec} seconds'\"",
            "azure":      "psql -U {db_user} -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state='idle' AND state_change < NOW() - INTERVAL '{max_idle_sec} seconds'\"",
        },
        "category": "remediation_intrusive",
        "blast_radius": 3,
        "requires_approval": True,
    ...
    },
    {
        ...
        "description": "List highest CPU/memory processes on the host.",
        "command": "ps aux --sort=-{sort_by} 2>/dev/null | head -{limit} || top -bn1 | tail -n +5 | head -{limit}",
        "command_variants": {
            "docker":     "ps aux --sort=-{sort_by} 2>/dev/null | head -{limit} || top -bn1 | tail -n +5 | head -{limit}",
            "ssh":        "ssh {host} sh -c 'ps aux --sort=-{sort_by} 2>/dev/null | head -{limit} || top -bn1 | tail -n +5 | head -{limit}'",
            "kubernetes": "ps aux --sort=-{sort_by} 2>/dev/null | head -{limit} || top -bn1 | tail -n +5 | head -{limit}",
            "vcenter":    "ps aux --sort=-{sort_by} 2>/dev/null | head -{limit} || top -bn1 | tail -n +5 | head -{limit}",
            "aws_ssm":    "ps aux --sort=-{sort_by} 2>/dev/null | head -{limit} || top -bn1 | tail -n +5 | head -{limit}",
            "azure":      "ps aux --sort=-{sort_by} 2>/dev/null | head -{limit} || top -bn1 | tail -n +5 | head -{limit}",
        },
        "category": "diagnostic",
        "blast_radius": 1,
        "requires_approval": False,
    ...
    },
    {
        ...
        "description": "Get PID, status, and resource usage for a named process on the host.",
        "command": "sh -c 'PID=$(pgrep -f {process_name} | head -1) && (ps -o pid,ppid,user,args 2>/dev/null | grep \"^ *$PID \" || ps | grep \"^ *$PID \") && cat /proc/\"$PID\"/status 2>/dev/null'",
        "command_variants": {
            "docker":     "sh -c 'PID=$(pgrep -f {process_name} | head -1) && (ps -o pid,ppid,user,args 2>/dev/null | grep \"^ *$PID \" || ps | grep \"^ *$PID \") && cat /proc/\"$PID\"/status 2>/dev/null'",
            "ssh":        "ssh {host} sh -c 'PID=$(pgrep -f {process_name} | head -1) && (ps -o pid,ppid,user,args 2>/dev/null | grep \"^ *$PID \" || ps | grep \"^ *$PID \") && cat /proc/\"$PID\"/status 2>/dev/null'",
            "kubernetes": "sh -c 'PID=$(pgrep -f {process_name} | head -1) && (ps -o pid,ppid,user,args 2>/dev/null | grep \"^ *$PID \" || ps | grep \"^ *$PID \") && cat /proc/\"$PID\"/status 2>/dev/null'",
            "vcenter":    "sh -c 'PID=$(pgrep -f {process_name} | head -1) && (ps -o pid,ppid,user,args 2>/dev/null | grep \"^ *$PID \" || ps | grep \"^ *$PID \") && cat /proc/\"$PID\"/status 2>/dev/null'",
            "aws_ssm":    "sh -c 'PID=$(pgrep -f {process_name} | head -1) && (ps -o pid,ppid,user,args 2>/dev/null | grep \"^ *$PID \" || ps | grep \"^ *$PID \") && cat /proc/\"$PID\"/status 2>/dev/null'",
            "azure":      "sh -c 'PID=$(pgrep -f {process_name} | head -1) && (ps -o pid,ppid,user,args 2>/dev/null | grep \"^ *$PID \" || ps | grep \"^ *$PID \") && cat /proc/\"$PID\"/status 2>/dev/null'",
        },
        "category": "diagnostic",
        "blast_radius": 1,
        "requires_approval": False,
    ...
    },
    {
        ...
        "description": "List active TCP/UDP connections and listening ports on the host.",
        "command": "ss -tunaop",
        "command_variants": {
            "docker":     "ss -tunaop",
            "ssh":        "ssh {host} ss -tunaop",
            "kubernetes": "ss -tunaop",
            "vcenter":    "ss -tunaop",
            "aws_ssm":    "ss -tunaop",
            "azure":      "ss -tunaop",
        },
    ...
    },
    {
        ...
        "description": "Delete log files older than N days on the host.",
        "command": "find {path} -name '*.log' -mtime +{days_to_retain} -delete",
        "command_variants": {
            "docker":     "find {path} -name '*.log' -mtime +{days_to_retain} -delete",
            "ssh":        "ssh {host} find {path} -name '*.log' -mtime +{days_to_retain} -delete",
            "kubernetes": "find {path} -name '*.log' -mtime +{days_to_retain} -delete",
    ...
    # ══════════════════════════════════════════════════════════
    {
        ...
        "description": "Send a POSIX signal to a named process on the host.",
        "command": "pkill -{signal} {process_name}",
        "command_variants": {
            "docker":     "pkill -{signal} {process_name}",
            "ssh":        "ssh {host} pkill -{signal} {process_name}",
            "kubernetes": "pkill -{signal} {process_name}",
            "vcenter":    "pkill -{signal} {process_name}",
            "aws_ssm":    "pkill -{signal} {process_name}",
            "azure":      "pkill -{signal} {process_name}",
        },
