...
│  Splunk / Datadog / Prometheus / PagerDuty webhooks  │
└──────────────────────────────────────────────────────┘
```
...
Storm detection runs on the `workflow_states` table, not on the raw event stream. By the time detection runs, watcher events and Splunk/Datadog webhook events have both been normalised into incidents by `EventQualificationService`. The storm can span both sources transparently.
