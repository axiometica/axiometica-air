...
**Connector types:** `servicenow`, `splunk`, `datadog`, `dynatrace`, `prometheus`, `pagerduty`, `zabbix`
...
**Endpoint:** `POST /api/webhooks/{connector_type}/{connector_id}`

This endpoint receives inbound alerts from external monitoring tools. The URL is shown in the Connector Hub UI after creating a connector. The platform normalises the payload and routes it through the qualification and incident pipeline.

Authentication: HMAC-SHA256 `X-Webhook-Signature` header (when `webhook_secret` is configured).
