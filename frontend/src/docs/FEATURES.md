...
**VerifierAgent** runs post-remediation checks after the runbook completes:
- Executes the verification steps defined in the runbook (metric threshold checks, health endpoint probes)
- Sets `remediation_outcome` to `succeeded` or `failed` based on results
- Attaches verification details to the incident's Remediation tab
...
Policies are defined with match criteria and enforcement rules. The PolicyBrokerAgent evaluates all policies in priority order and applies the first match:
...
2. The incident appears in the **CAB approval queue** on the dashboard
...
The dashboard includes a visual policy rule builder for creating and modifying governance policies. Fields include match criteria dropdowns, approval toggle, blast radius numeric input, and allowed actions multi-select. Changes take effect immediately for newly created incidents.
...
| `notify` | Emit a Slack or webhook notification mid-workflow | `continue` |
...
Visual rule builder for governance policies. Supports match criteria dropdowns (anomaly type, service, environment, severity), approval required toggle, blast radius numeric limit, and allowed actions multi-select. Policies can be created, edited, or deleted from the UI.
...
| PagerDuty certified connector | Implemented |
...
| TuningAgent — 7-check analysis of resolved incidents | Implemented |
| Live config application (no restart required) | Implemented |
| Scheduled tuning runs | Implemented |
| **Summaries** | |
...
| Step types: diagnostic, action, verification, decision, notify | Implemented |
...
| Advanced analytics (MTTR trends, service health scorecards) | Planned |
